...
- Cluster manifest (Application/AppProject/Helm values) — `agile-server/bootstrap/applications.yaml` + `agile-server/manifests/` が SSOT
...
`agile-server/bootstrap/applications.yaml` で Helm chart のバージョンを `targetRevision: 0.14.1` のように pin するのは OK (これは git revision ではなく chart version、ArgoCD の Helm source で同フィールドが両用途を兼ねている)。arc-controller、traefik、zot 等が該当。
