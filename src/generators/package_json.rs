...
pub fn generate_full_package_json(
    ...
    // Load existing package.json to preserve manual edits in dependencies/scripts
    let mut existing_obj = if package_json_path.exists() {
    ...
    // Scripts — preserve existing if they exist, otherwise use resolved defaults
    let final_scripts = if let Some(ref mut existing) = existing_obj
        && let Some(existing_scripts) = existing.get("scripts").and_then(|v| v.as_object())
    {
        existing_scripts.clone()
    } else if !resolved_data.scripts.is_empty() {
        resolved_data
            .scripts
            .iter()
            .map(|(k, v)| (k.clone(), json!(v)))
    ...
    if !final_scripts.is_empty() {
        obj.insert(
            "scripts".to_string(),
            serde_json::Value::Object(final_scripts),
        );
    }

    // Dependencies — ALWAYS preserve existing if they exist.
    // Airis should not overwrite dependencies managed by pnpm/human.
    let final_deps = if let Some(ref mut existing) = existing_obj
        && let Some(existing_deps) = existing.get("dependencies").and_then(|v| v.as_object())
    {
        existing_deps.clone()
    } else if !resolved_data.deps.is_empty() {
        resolved_data
            .deps
            .iter()
            .map(|(k, v)| {
    ...
    if !final_deps.is_empty() {
        obj.insert(
            ...
            serde_json::Value::Object(final_deps),
        );
    }

    let final_dev_deps = if let Some(ref mut existing) = existing_obj
        && let Some(existing_dev_deps) = existing.get("devDependencies").and_then(|v| v.as_object())
    {
        existing_dev_deps.clone()
    } else if !resolved_data.dev_deps.is_empty() {
        resolved_data
            .dev_deps
            .iter()
            .map(|(k, v)| {
    ...
    if !final_dev_deps.is_empty() {
        obj.insert(
            "devDependencies".to_string(),
            serde_json::Value::Object(final_dev_deps),
        );
    }
    ...
    obj.insert(
        ...
        json!({
            "by": "airis gen",
            "mode": "hybrid",
            "warning": "Core fields (name, version, type, etc.) are managed by airis gen. dependencies and scripts are preserved if they exist."
        }),
    );
    ...
    println!("  ✓ Updated {}", package_json_path.display());
