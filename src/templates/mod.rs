        ...
        for svc in manifest.service.values() {
            for vol in &svc.volumes {
                ...
                if let Some(name) = vol.split(':').next()
                    && !name.starts_with('.') && !name.starts_with('/') && !volume_names.contains(&name.to_string()) {
...
const NPMRC_TEMPLATE: &str = include_str!("../../templates/hbs/npmrc.hbs");

const PACKAGE_JSON_TEMPLATE: &str = include_str!("../../templates/hbs/package.json.hbs");

const PNPM_WORKSPACE_TEMPLATE: &str = include_str!("../../templates/hbs/pnpm-workspace.hbs");


const DOCKER_COMPOSE_TEMPLATE: &str = include_str!("../../templates/hbs/docker-compose.hbs");
