...
impl TemplateEngine {
    ...
    pub fn render_service_dockerfile(
        ...
        // Python projects use a completely different Dockerfile structure
        if variant == "python" {
            return self.render_python_dockerfile(app, deploy, path, port);
        }
    ...
    /// Render a Python service Dockerfile from manifest config.
    ///
    /// Uses pyproject.toml for dependency management.
    /// Supports optional GPU (CUDA) via `[app.deploy] gpu = true`.
    fn render_python_dockerfile(
        &self,
        app: &crate::manifest::ProjectDefinition,
        ...
        let python_image = deploy
            .base_image
            .as_deref()
            .unwrap_or(crate::channel::defaults::PYTHON_IMAGE);
        let health_path = deploy.health_path.as_deref().unwrap_or("/health");
        let health_interval = &deploy.health_interval;
        let health_timeout = deploy.health_timeout.as_deref().unwrap_or("10s");
        let health_start_period = deploy.health_start_period.as_deref().unwrap_or("30s");
        ...
        let entrypoint = deploy
            .entrypoint
            .clone()
            .unwrap_or_else(|| "src.main:app".to_string());

        // Extra system packages (e.g., ffmpeg, libsndfile1)
        let extra_apt: Vec<String> = deploy
            .extra_apk
            .iter()
            .map(|s| format!("    {} \\", s))
            .collect();
        ...
        .unwrap();
        writeln!(out, "#").unwrap();
        writeln!(out, "# Variant: python | Package: {}", app.name).unwrap();
        writeln!(out).unwrap();
        ...
        .unwrap();
        writeln!(out, "FROM {} AS runtime", python_image).unwrap();
        writeln!(out, "WORKDIR /app").unwrap();
        writeln!(out).unwrap();
        writeln!(out, "COPY --from=deps /usr/local /usr/local").unwrap();
        writeln!(out, "COPY {}/src ./src", path).unwrap();
        writeln!(out).unwrap();

        writeln!(out, "RUN useradd --create-home --shell /bin/bash worker").unwrap();
        writeln!(out, "USER worker").unwrap();
        writeln!(out).unwrap();
        writeln!(out, "ENV PYTHONUNBUFFERED=1").unwrap();
        writeln!(out).unwrap();
        ...
        // CMD: uvicorn for web, or python -m for workers
        if entrypoint.contains(':') {
            // ASGI app (e.g., "src.main:app")
            writeln!(
                out,
                "CMD [\"python3\", \"-m\", \"uvicorn\", \"{}\", \"--host\", \"0.0.0.0\", \"--port\", \"{}\"]",
                entrypoint, port
            )
            .unwrap();
        } else {
            // Module execution (e.g., "src.worker")
            writeln!(out, "CMD [\"python3\", \"-m\", \"{}\"]", entrypoint).unwrap();
        }
...
"#,
    ...
    fn prepare_docker_compose_data(
        &self,
        manifest: &Manifest,
        root: &str,
