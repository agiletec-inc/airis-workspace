...
impl TemplateEngine {
    ...
    pub fn render_service_dockerfile(
        ...
        let extra_apk_str: String = deploy.extra_apk.iter()
            .map(|pkg| format!(" {}", pkg))
            .collect();
        ...
        writeln!(out, "# DO NOT EDIT - change manifest.toml [app.deploy] instead.").unwrap();
        writeln!(out, "#").unwrap();
        writeln!(out, "# Single image for all environments (local/stg/prd).").unwrap();
        writeln!(out, "# Environment differences are handled by env vars (Doppler) and compose command.").unwrap();
        writeln!(out, "#").unwrap();
        writeln!(out, "# Variant: {} | Package: {}/{}", variant, scope, app.name).unwrap();
        writeln!(out).unwrap();

        // base stage
        writeln!(out, "# ── base ──────────────────────────────────────────────────────────").unwrap();
        writeln!(out, "FROM {} AS base", node_image).unwrap();
        writeln!(out, "RUN apk add --no-cache libc6-compat{} \\", extra_apk_str).unwrap();
        writeln!(out, " && {}", pm_install_cmd).unwrap();
        writeln!(out).unwrap();

        // pruner stage
        writeln!(out, "# ── pruner ────────────────────────────────────────────────────────").unwrap();
        ...
        // deps stage
        writeln!(out, "# ── deps ──────────────────────────────────────────────────────────").unwrap();
        writeln!(out, "FROM base AS deps").unwrap();
        writeln!(out, "WORKDIR /app").unwrap();
        writeln!(out, "COPY --from=pruner /app/out/json/ .").unwrap();
        writeln!(out, "RUN --mount=type=cache,id=pnpm,target=/pnpm/store pnpm install").unwrap();
        writeln!(out).unwrap();
        ...
        writeln!(out, "RUN pnpm turbo run build --filter={}/{}", scope, app.name).unwrap();
        if is_node {
            writeln!(out, "RUN pnpm deploy --legacy --filter={}/{} --prod /app/deploy", scope, app.name).unwrap();
        }
        writeln!(out).unwrap();
        ...
        if is_nextjs {
            ...
            writeln!(out, "COPY --from=builder /app/{}/.next/static ./{}/.next/static", path, path).unwrap();
            writeln!(out, "COPY --from=builder /app/{}/public ./{}/public", path, path).unwrap();
        } else {
            writeln!(out, "COPY --from=builder /app/{}/dist ./{}/dist", path, path).unwrap();
            writeln!(out, "COPY --from=builder /app/deploy/package.json ./{}/", path).unwrap();
            writeln!(out, "COPY --from=builder /app/deploy/node_modules ./{}/node_modules", path).unwrap();
        }
        writeln!(out).unwrap();

        writeln!(out, "RUN addgroup -g 1001 -S nodejs && adduser -S nodejs -u 1001").unwrap();
        ...
        if !is_worker {
            ...
            writeln!(out).unwrap();
            writeln!(out, "HEALTHCHECK --interval={} --timeout={} --start-period={} --retries={} \\", health_interval, health_timeout, health_start_period, health_retries).unwrap();
            writeln!(out, "  CMD wget -q --spider http://localhost:{}{} || exit 1", port, health_path).unwrap();
            writeln!(out).unwrap();
        }
    ...
    pub fn render_docker_compose(&self, manifest: &Manifest) -> Result<String> {
        ...
        if let Some(services) = services {
            for svc in services {
                ...
                if let Some(gpu) = svc["gpu"].as_object() {
                    let driver = gpu.get("driver").and_then(|v| v.as_str()).unwrap_or("nvidia");
                ...
                if let Some(hp) = svc["health_path"].as_str() {
                    ...
                    let interval = svc.get("health_interval")
                        .and_then(|v| v.as_str())
                        .unwrap_or("30s");
                    let timeout = svc.get("health_timeout")
                        .and_then(|v| v.as_str())
                        .unwrap_or("10s");
                    let retries = svc.get("health_retries")
                        .and_then(|v| v.as_u64())
                        .unwrap_or(3);
                    let start_period = svc.get("health_start_period")
                        .and_then(|v| v.as_str())
                        .unwrap_or("40s");
        ...
        if let Some(defs) = network_defs {
            ...
            for (net_name, net_val) in defs {
                let external = net_val.get("external").and_then(|v| v.as_bool()).unwrap_or(false);
    ...
    pub fn render_release_workflow(&self, manifest: &Manifest) -> Result<String> {
        ...
        Ok(format!(r#"# Auto-generated by airis gen
