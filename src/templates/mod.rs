...
impl TemplateEngine {
    ...
    pub fn render_claude_md(&self, manifest: &Manifest) -> Result<String> {
        ...
        // Workspace Workflow
        sections.push(self.render_workflow_section(manifest)?);
    ...
    pub fn render_claude_md_sections(&self, manifest: &Manifest, sections: &[&str]) -> Result<String> {
        ...
        for (i, section) in sections.iter().enumerate() {
            ...
            match *section {
                ...
                "workflow" => {
                    output.push_str(&self.render_workflow_section(manifest)?);
                }
                _ => {}
            }
        }
    ...
    /// Render the Workspace Workflow section for CLAUDE.md
    fn render_workflow_section(&self, manifest: &Manifest) -> Result<String> {
        let mut lines = vec!["## Workspace Workflow\n".to_string()];

        // Single Source of Truth
        lines.push("### Single Source of Truth\n".to_string());
        lines.push(
            "`manifest.toml` is the **only file you should edit**. \
             All other configuration files are auto-generated by `airis generate files`.\n"
                .to_string(),
        );

        // Auto-generated files (do not edit)
        lines.push("### Auto-Generated Files (DO NOT EDIT)\n".to_string());
        lines.push("| File | Generated From |".to_string());
        lines.push("|------|---------------|".to_string());
        lines.push("| `package.json` | `manifest.toml` |".to_string());
        lines.push("| `pnpm-workspace.yaml` | `manifest.toml` |".to_string());
        lines.push("| `compose.yml` | `manifest.toml` |".to_string());
        lines.push("| `.env.example` | `manifest.toml [env]` |".to_string());
        lines.push("| `CLAUDE.md` | `manifest.toml` |".to_string());
        lines.push(String::new());

        // Catalog version management (only if catalog is non-empty)
        if !manifest.packages.catalog.is_empty() {
            lines.push("### Catalog Version Management\n".to_string());
            lines.push(
                "Dependencies use `\"catalog:\"` in package.json, resolved from `[packages.catalog]` in manifest.toml. \
                 To update a dependency version, edit `manifest.toml` and run `airis generate files`.\n"
                    .to_string(),
            );
        }

        // Development flow
        lines.push("### Development Flow\n".to_string());
        lines.push("```bash".to_string());
        lines.push("# 1. Edit manifest.toml (the only file you edit)".to_string());
        lines.push("vim manifest.toml".to_string());
        lines.push(String::new());
        ...
        lines.push("| Hook | Purpose | Status |".to_string());
        lines.push("|------|---------|--------|".to_string());
        lines.push(
            "| `pre-commit` | Version auto-bump, .env guard, node_modules guard | Active |"
                .to_string(),
        );
        lines.push(
            "| `pre-push` | lint / typecheck / build | Template (commented out) |".to_string(),
        );
        lines.push(String::new());
        lines.push(
            "Install hooks: `airis hooks install`\n".to_string(),
        );
        ...
        lines.push(
            "**NEVER** use `--no-verify` to skip Git hooks. \
             If a hook fails, fix the underlying issue instead.\n"
                .to_string(),
        );
...
"#;
    ...
    #[test]
    fn test_render_workflow_section_basic() {
        let manifest = minimal_manifest();
        let engine = TemplateEngine::new().unwrap();
        let result = engine.render_workflow_section(&manifest).unwrap();

        // Should contain the section header
        assert!(result.contains("## Workspace Workflow"));

        // Should contain Single Source of Truth
        assert!(result.contains("Single Source of Truth"));
        assert!(result.contains("manifest.toml"));

        // Should contain auto-generated files table
        assert!(result.contains("Auto-Generated Files"));
        assert!(result.contains("DO NOT EDIT"));
        assert!(result.contains("package.json"));

        // Should contain development flow
        assert!(result.contains("Development Flow"));
        assert!(result.contains("airis generate files"));
        assert!(result.contains("airis doctor"));

        // Should contain git hooks table
        assert!(result.contains("pre-commit"));
        assert!(result.contains("pre-push"));
        assert!(result.contains("airis hooks install"));

        // Should contain --no-verify prohibition
        assert!(result.contains("--no-verify"));
    }

    #[test]
    fn test_render_workflow_section_with_catalog() {
        let toml_str = r#"
[workspace]
name = "test-project"
service = "workspace"
...
"#;
        ...
        // Should contain catalog management section
    ...
    #[test]
    fn test_render_workflow_section_no_catalog() {
        let manifest = minimal_manifest();
        let engine = TemplateEngine::new().unwrap();
        let result = engine.render_workflow_section(&manifest).unwrap();

        // Should NOT contain catalog section when catalog is empty
        assert!(!result.contains("Catalog Version Management"));
    }

    #[test]
    fn test_render_claude_md_sections_workflow() {
        let manifest = minimal_manifest();
        let engine = TemplateEngine::new().unwrap();
        let result = engine
            .render_claude_md_sections(&manifest, &["workflow"])
            .unwrap();
    ...
    #[test]
    fn test_render_claude_md_includes_workflow() {
        let manifest = minimal_manifest();
        let engine = TemplateEngine::new().unwrap();
        let result = engine.render_claude_md(&manifest).unwrap();

        // Full CLAUDE.md should include the workflow section
        assert!(result.contains("Workspace Workflow"));
        assert!(result.contains("Single Source of Truth"));
        assert!(result.contains("--no-verify"));
    }
