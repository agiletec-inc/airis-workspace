...
pub fn get_ownership(path: &Path) -> Ownership {
    ...
    match path_str.as_ref() {
        ...
        // User-owned by default, but Tool-owned if they have the airis marker
        "compose.yml" | "compose.yaml" | "docker-compose.yml" | "workspace/compose.yml"
        | "workspace/compose.yaml" | "workspace/docker-compose.yml" => {
            if !path.exists() {
                return Ownership::Tool; // Allow creation if it doesn't exist
            }
            if let Ok(content) = std::fs::read_to_string(path)
                && (content.contains("# Generated by airis-workspace")
