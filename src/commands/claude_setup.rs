...
# Catches host path contamination, bind mount abuse, and pnpm store misconfig.
...
# Skip documentation and rule files
if [[ "$FILE_PATH" =~ \.(md|txt|rst)$ ]]; then
  exit 0
fi

# --- Rule 1: Block host paths in Dockerfile / compose / CI ---
if [[ "$BASENAME" =~ ^(Dockerfile|Dockerfile\..+|compose\.ya?ml|docker-compose\.ya?ml|ci\.ya?ml|deploy\.ya?ml)$ ]]; then

  # Host-specific absolute paths
  if echo "$NEW_CONTENT" | grep -qE '(/Users/|/home/[a-z]|~/)'; then
    cat >&2 <<'BLOCK'
BLOCKED: Host path in Docker/CI file

  You wrote a host-specific path (/Users/..., /home/..., ~/) into a
  Dockerfile, compose.yml, or CI config. This WILL fail in CI.
...
BLOCK
  ...
  if echo "$NEW_CONTENT" | grep -qE '\.pnpm-store|\.pnpm/store|node_modules/\.pnpm'; then
    cat >&2 <<'BLOCK'
BLOCKED: Local pnpm store path in Docker/CI file
...
# --- Rule 2: Block bind mount of node_modules in compose ---
if [[ "$BASENAME" =~ ^(compose\.ya?ml|docker-compose\.ya?ml)$ ]]; then
  if echo "$NEW_CONTENT" | grep -qE '\./node_modules\s*:|\./\.pnpm'; then
    cat >&2 <<'BLOCK'
BLOCKED: Bind mount of node_modules in compose file
...
# --- Rule 3: Block PNPM_STORE_DIR pointing to host paths ---
if echo "$NEW_CONTENT" | grep -qE 'PNPM_STORE_DIR\s*=\s*["\x27]?\.(\/|pnpm)|PNPM_STORE_DIR\s*=\s*["\x27]?(/Users|/home|~/)'; then
  cat >&2 <<'BLOCK'
BLOCKED: PNPM_STORE_DIR set to host-local path

  PNPM_STORE_DIR must point to a named volume mount.

  Correct: PNPM_STORE_DIR=/pnpm/store
  Wrong:   PNPM_STORE_DIR=./.pnpm-store
  Wrong:   PNPM_STORE_DIR=/Users/.../.pnpm-store
BLOCK
  exit 2
fi
...
mod tests {
    ...
    fn test_generate_edit_guard_script() {
        ...
        assert!(script.contains("exit 2"));
        // Should skip documentation files
        assert!(script.contains("md|txt|rst"));
    }
