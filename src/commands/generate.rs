...
pub fn preview_from_manifest(manifest: &Manifest) -> Result<()> {
    ...
    let files_to_check = vec![
        ...
        // CI/CD workflows are project-owned — not generated by airis gen
    ];
...
pub fn sync_from_manifest_with_force(manifest: &Manifest, force: bool) -> Result<()> {
    ...
    //
    // Note: CI/CD workflows (.github/workflows/) are NOT generated — they are project-owned.
    // Each project should manage workflows directly for full GHA feature access.
    ...
    // CI/CD workflows are project-owned — not listed here
    if is_rust_project {
        println!();
    ...
    let deploy_count = manifest.app.iter()
        ...
        .count();
    if deploy_count > 0 {
        println!("   - {} service Dockerfiles (turbo prune)", deploy_count);
    }
...
// generate_github_workflows removed — CI/CD workflows are project-owned, not tool-generated.
