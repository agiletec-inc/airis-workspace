...
pub fn install() -> Result<()> {
    ...
    println!(
        "{}",
        "Now all commands (pnpm, npm, node, etc.) will run inside Docker!".cyan()
    );
...
pub fn exec(cmd: &str, args: &[String]) -> Result<()> {
    ...
    // Build command
    let mut docker_args = vec!["compose".to_string(), "-f".to_string(), compose.clone()];

    // Check if service is running
    let status = Command::new("docker")
        .args([
            "compose", "-f", compose, "ps", "--status", "running", &service,
        ])
        .output()?;
    ...
    docker_args.extend(["-w".to_string(), workdir, service, cmd.to_string()]);
...
pub fn list() -> Result<()> {
    ...
    if !bin_dir.exists() {
        println!(
            "{}",
            "No shims installed. Run 'airis shim install' first.".yellow()
        );
        return Ok(());
    }
